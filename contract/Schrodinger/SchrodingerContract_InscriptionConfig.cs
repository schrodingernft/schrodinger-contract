...
public partial class SchrodingerContract
{
    ...
    {
        Assert(
            !string.IsNullOrWhiteSpace(input.Tick) && input.LossRate > 0 &&
            input.LossRate <= SchrodingerContractConstants.Denominator &&
            input.CommissionRate > 0 && input.CommissionRate <= SchrodingerContractConstants.Denominator,
            "Invalid input.");
        var inscription = CheckInscriptionExistAndPermission(input.Tick);
    ...
    {
        Assert(!string.IsNullOrWhiteSpace(input.Tick) && input.Recipient != null && input.Recipient.Value.Any(),
            "Invalid input.");
        var inscription = CheckInscriptionExistAndPermission(input.Tick);
    ...
    {
        Assert(!string.IsNullOrWhiteSpace(input.Tick) && input.Admin != null && input.Admin.Value.Any(),
            "Invalid input.");
        var inscription = CheckInscriptionExistAndPermission(input.Tick);
